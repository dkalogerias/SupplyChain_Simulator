...
###############################################################################
def dataPrep(H):
    # We will modify this to dataPrep(H, sth) when we want to set the value of our tunable parameter to sth
    ...
    for line in ChainFile:
        ...
        if attList[-1] == -1:
            ...
            #----------------------Parameters---------------------#
            # Penalties
            # Penalties are different at different times
            # and different for each parent supplier
            penalties = []
            for t in range(H):
                penalties.append(dict(zip(parentList, 5*np.random.rand(len(parentList)))))
            # Stock Cost per Unit
            ...
            SupplierDict[attList[0]] = Supplier(attList[0], attList[1], attList[2], # Label, Lat, Long
                                                ...
                                                # penalties, KI, KO, KPro, KPur, theta
                                                penalties, KI, KO, KPro, KPur, 0, 0)
    ...
    maxtreedepth = max([SupplierDict[ID].treeDepth for ID in uniqueID])
    # fix penalties
    # the penalty cost is higher close to the root node
    for ID in uniqueID:
        penalties = []
        for t in range(H):
            penalties.append(dict(zip(SupplierDict[ID].ParentLabels, (0.01*(maxtreedepth - SupplierDict[ID].treeDepth + 1)) * np.ones(SupplierDict[ID].NumberOfParents))))
        SupplierDict[ID].Penalty  = penalties
