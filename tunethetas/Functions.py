...
def Plan_LookaheadMIP(H, Hdict, ParentLabels, ChildrenLabels, ChildrenTrTimes, # remove number of children because no use
                      ...
                      penalty, KO, KI, KPro, KPur, Q, NumDiff, theta):
    # C = Production Cap, Q = Projected demands from children, NumDiff = Number of different parts
    # theta is the tunable parameter
    ###############################################################################
    ...
    for t in range(H):
        ...
        temp = temp + lpSum(penalty[t][par] * U_Vars[t][par] for par in ParentLabels) \
    ...
    for t in range(H):
        ...
        if t == 0:
            ...
            prob += RO_Vars[t] - RO_Previous - X_Vars[t] + lpSum(theta*D[par][t] - U_Vars[t][par] for par in ParentLabels) == 0
        for par in ParentLabels:
            ...
            if t < Hdict[par]:
                if t == 0:
                    ...
                    prob += U_Vars[t][par] - theta*D[par][t] <= 0
    ...
    # compute the cost of today
    costtoday = 0
    for par in ParentLabels:
        costtoday = costtoday + penalty[0][par]*UnMet[par][0]
    costtoday = costtoday + (KO*Out)
    for part in range(NumDiff):
        costtoday = costtoday + (KI[part+1]*In[part+1])
    costtoday = costtoday + (KPro*X_Values[0])
    for child in ChildrenLabels:
        costtoday = costtoday + (KPur[child]*UPD_Values[child][0])

    return X_Values, UPD_Values, In, Out, UnMet, costtoday
#print('done this')
