...
class Supplier:
    ...
    def __init__(self, Label, Lat, Long,
                 ...
                 Penalty, KI, KO, KPro, KPur, cost, theta): # Last line: Parameters
        # the last variable added is cost: refers to the cost of the objective function in the optimization
        self.cost = cost
        self.Label = Label
        self.Lat = Lat
        self.Long = Long
    ...
    def _SupplierUpdate(self, DataFromChildren, DataFromParent):
        ...
        X_Values, UpStreamDemand, In, Out, Unmet, thiscost = \
        Plan_LookaheadMIP(int(self.Horizon), self.HorizonDict, self.ParentLabels, self.ChildrenLabels,
                          ...
                          self.Penalty, self.KO, self.KI, self.KPro, self.KPur,
                          self.ProductDemands, self.NumberOfDiffParts, self.theta)

        self.cost = self.cost + thiscost

        if self.Label == 1054:
            print("Quantity produced: ", X_Values)
            print("Quantity stored in input inventory: ", In)
            print("Quantity stored in output inventory: ", Out)
            print("Unmet demand of parents: ", Unmet)
            print("Upstream demand requested to children: ", UpStreamDemand)
